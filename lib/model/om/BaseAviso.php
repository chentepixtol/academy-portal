...
abstract class BaseAviso extends BaseObject  implements Persistent {
  ...
  const PEER = 'AvisoPeer';
	...
	/**
	 * Initializes internal state of BaseAviso object.
	 * @see        applyDefaults()
	 */
	...
	public function setActivo($v)
	{
		...
		if ($this->activo !== $v) {
			$this->activo = $v;
			$this->modifiedColumns[] = AvisoPeer::ACTIVO;
		}
	...
	public function setFecha($v)
	{
		...
		if ( $this->fecha !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = AvisoPeer::FECHA;
			}
	...
	public function setSlug($v)
	{
		...
		if ($this->slug !== $v) {
			$this->slug = $v;
			$this->modifiedColumns[] = AvisoPeer::SLUG;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 6; // 6 = AvisoPeer::NUM_COLUMNS - AvisoPeer::NUM_LAZY_LOAD_COLUMNS).
    ...
    foreach (sfMixer::getCallables('BaseAviso:delete:pre') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseAviso:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseAviso:save:pre') as $callable)
    {
      ...
      if (is_int($affectedRows))
      {
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BaseAviso:save:post') as $callable)
    {
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(AvisoPeer::ACTIVO)) $criteria->add(AvisoPeer::ACTIVO, $this->activo);
		if ($this->isColumnModified(AvisoPeer::FECHA)) $criteria->add(AvisoPeer::FECHA, $this->fecha);
		if ($this->isColumnModified(AvisoPeer::SLUG)) $criteria->add(AvisoPeer::SLUG, $this->slug);
  ...
  {
    if (!$callable = sfMixer::getCallable('BaseAviso:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BaseAviso::%s', $method));
    }
