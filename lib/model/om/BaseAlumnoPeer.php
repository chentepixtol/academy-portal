...
abstract class BaseAlumnoPeer {
	...
	const APELLIDO_PATERNO = 'alumno.APELLIDO_PATERNO';
	...
	/**
	 * An identiy map to hold any loaded instances of Alumno objects.
	...
	private static $fieldNames = array (
		BasePeer::TYPE_PHPNAME => array ('Id', 'Username', 'Password', 'Email', 'Nombre', 'ApellidoPaterno', 'ApellidoMaterno', 'Boleta', ),
		BasePeer::TYPE_STUDLYPHPNAME => array ('id', 'username', 'password', 'email', 'nombre', 'apellidoPaterno', 'apellidoMaterno', 'boleta', ),
		BasePeer::TYPE_COLNAME => array (self::ID, self::USERNAME, self::PASSWORD, self::EMAIL, self::NOMBRE, self::APELLIDO_PATERNO, self::APELLIDO_MATERNO, self::BOLETA, ),
		BasePeer::TYPE_FIELDNAME => array ('id', 'username', 'password', 'email', 'nombre', 'apellido_paterno', 'apellido_materno', 'boleta', ),
	...
	private static $fieldKeys = array (
		BasePeer::TYPE_PHPNAME => array ('Id' => 0, 'Username' => 1, 'Password' => 2, 'Email' => 3, 'Nombre' => 4, 'ApellidoPaterno' => 5, 'ApellidoMaterno' => 6, 'Boleta' => 7, ),
		BasePeer::TYPE_STUDLYPHPNAME => array ('id' => 0, 'username' => 1, 'password' => 2, 'email' => 3, 'nombre' => 4, 'apellidoPaterno' => 5, 'apellidoMaterno' => 6, 'boleta' => 7, ),
		BasePeer::TYPE_COLNAME => array (self::ID => 0, self::USERNAME => 1, self::PASSWORD => 2, self::EMAIL => 3, self::NOMBRE => 4, self::APELLIDO_PATERNO => 5, self::APELLIDO_MATERNO => 6, self::BOLETA => 7, ),
		BasePeer::TYPE_FIELDNAME => array ('id' => 0, 'username' => 1, 'password' => 2, 'email' => 3, 'nombre' => 4, 'apellido_paterno' => 5, 'apellido_materno' => 6, 'boleta' => 7, ),
		BasePeer::TYPE_NUM => array (0, 1, 2, 3, 4, 5, 6, 7, )
	);
	...
	public static function addSelectColumns(Criteria $criteria)
	{
		...
		$criteria->addSelectColumn(AlumnoPeer::USERNAME);

		$criteria->addSelectColumn(AlumnoPeer::PASSWORD);

		$criteria->addSelectColumn(AlumnoPeer::EMAIL);

		$criteria->addSelectColumn(AlumnoPeer::NOMBRE);

		$criteria->addSelectColumn(AlumnoPeer::APELLIDO_PATERNO);

		$criteria->addSelectColumn(AlumnoPeer::APELLIDO_MATERNO);

		$criteria->addSelectColumn(AlumnoPeer::BOLETA);
    ...
    foreach (sfMixer::getCallables('BaseAlumnoPeer:doCount:doCount') as $callable)
    {
      call_user_func($callable, 'BaseAlumnoPeer', $criteria, $con);
    }
    ...
    foreach (sfMixer::getCallables('BaseAlumnoPeer:doSelectStmt:doSelectStmt') as $callable)
    {
      call_user_func($callable, 'BaseAlumnoPeer', $criteria, $con);
    }
	 ...
	 *
	 * @param      Alumno $value A Alumno object.
	 ...
	 *
	 * @param      mixed $value A Alumno object or a primary key value.
	 */
	 ...
	 * @return     Alumno Found object or NULL if 1) no instance exists for specified key or 2) instance pooling has been disabled.
  ...
  static public function getUniqueColumnNames()
  {
    ...
    foreach (sfMixer::getCallables('BaseAlumnoPeer:doInsert:pre') as $callable)
    {
      $ret = call_user_func($callable, 'BaseAlumnoPeer', $values, $con);
      if (false !== $ret)
      {
        return $ret;
      }
    ...
    foreach (sfMixer::getCallables('BaseAlumnoPeer:doInsert:post') as $callable)
    {
      call_user_func($callable, 'BaseAlumnoPeer', $values, $con, $pk);
    }
    ...
    foreach (sfMixer::getCallables('BaseAlumnoPeer:doUpdate:pre') as $callable)
    {
      $ret = call_user_func($callable, 'BaseAlumnoPeer', $values, $con);
      if (false !== $ret)
      {
    ...
    foreach (sfMixer::getCallables('BaseAlumnoPeer:doUpdate:post') as $callable)
    {
      call_user_func($callable, 'BaseAlumnoPeer', $values, $con, $ret);
    }
...
//
// NOTE: This static code cannot call methods on the AlumnoPeer class, because it is not defined yet.
// If you need to use overridden methods, you can add this code to the bottom of the AlumnoPeer class:
//
// Propel::getDatabaseMap(AlumnoPeer::DATABASE_NAME)->addTableBuilder(AlumnoPeer::TABLE_NAME, AlumnoPeer::getMapBuilder());
//
